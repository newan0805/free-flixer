...
const Hero = () => {
  ...
  useEffect(() => {
    if (featuredMovie) {
      setIsInMyList(myList.isInList(featuredMovie.id, featuredMovie.media_type === 'movie' ? 'movie' : 'tv'));
    }
  }, [featuredMovie]);
  ...
  const handleMyListToggle = () => {
    ...
    const listItem = {
      ...
      first_air_date: featuredMovie.first_air_date,
      vote_average: featuredMovie.vote_average,
      genre_ids: featuredMovie.genre_ids || []
    };

    if (isInMyList) {
      myList.removeItem(featuredMovie.id, featuredMovie.media_type === 'movie' ? 'movie' : 'tv');
      setIsInMyList(false);
    } else {
  ...
  const handleMoreInfo = () => {
    if (featuredMovie) {
      const type = featuredMovie.media_type === 'movie' ? 'movie' : 'tv';
      router.push(`/${type}/${featuredMovie.id}`);
    }
  };
